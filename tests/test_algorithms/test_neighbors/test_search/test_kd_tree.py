...
class TestKDTreeQuery:
    ...
    def test_query_k_exceeds_n_samples(self):
        ...
        assert len(dists) == 3  # capped at n_samples
...
class TestKDTreeQueryRadius:
    ...
    def test_query_radius_finds_close_points(self):
        ...
        # Points at (0,0), (1,0), (0,1) should be within radius 1.5
...
class TestKDTreeConsistency:
    """Tests comparing KDTree results with brute force."""

    def test_matches_brute_force(self):
        """Test that KDTree query matches brute force for known data."""
        np.random.seed(42)
        ...
        tree = KDTree(leaf_size=5)
        tree.build(X)
        kd_dists, kd_indices = tree.query(query, k=5)
        ...
        np.testing.assert_allclose(kd_dists, bf_dists, atol=1e-10)
