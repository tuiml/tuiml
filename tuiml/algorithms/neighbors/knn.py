...
from tuiml.algorithms.neighbors.search import (
    NearestNeighborSearch, BruteForceSearch, KDTree, BallTree
)
from tuiml._cpp_ext import neighbors as _cpp_nn


def _inverse_weights(distances: np.ndarray, squared: bool = False) -> np.ndarray:
    """Inverse-distance weights without divide-by-zero warnings.

    Exact matches (``distance == 0``) receive a large fixed weight (1e10) so
    they dominate the vote, matching a plain ``1 / d`` rule.
    """
    denominators = distances ** 2 if squared else distances
    weights = np.zeros_like(distances)
    np.divide(1.0, denominators, out=weights, where=denominators > 0)
    weights[denominators == 0] = 1e10
    return weights
...
class KNearestNeighborsClassifier(Classifier):
    ...
    **Weighting schemes:**
    ...
    distance_weighting : {'uniform', 'distance', 'distance_squared', 'similarity'}, default='uniform'
    ...
    auto_select_k : bool, default=False
    ...
    **When to use KNearestNeighborsClassifier:**
    ...
    def __init__(self, k: int = 1,
        ...
        distance_weighting : str, default='uniform'
            ...
            ``'distance_squared'``, or ``'similarity'``.
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "distance_weighting": {
                "type": "string",
                "default": "uniform",
                "enum": ["uniform", "distance", "distance_squared", "similarity"],
            ...
            "auto_select_k": {
                "type": "boolean",
                "default": False,
                "description": "Use leave-one-out cross-validation to select k"
            },
            "leaf_size": {
                "type": "integer",
                "default": 30,
    ...
    def _compute_weights(self, distances: np.ndarray) -> np.ndarray:
        ...
        if self.distance_weighting == 'uniform':
            ...
            return _inverse_weights(distances)

        elif self.distance_weighting == 'distance_squared':
            return _inverse_weights(distances, squared=True)
    ...
    def _auto_select_k(self, X: np.ndarray, y: np.ndarray) -> int:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "KNearestNeighborsClassifier":
        ...
        # Leave-one-out cross-validation to find optimal k if requested
        if self.auto_select_k:
            self.k = self._auto_select_k(X, y)
    ...
    def _batch_predict_core(self, X: np.ndarray):
        ...
        if self.distance_weighting == 'distance':
            weights = _inverse_weights(distances)
        elif self.distance_weighting == 'distance_squared':
            weights = _inverse_weights(distances, squared=True)
        elif self.distance_weighting == 'similarity':
